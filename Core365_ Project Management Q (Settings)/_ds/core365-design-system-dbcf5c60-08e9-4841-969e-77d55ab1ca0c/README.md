...
| **Mono font** | **JetBrains Mono** (code, hex values, tabular data) |
...
| Fonts | `fonts/Manrope-VariableFont_wght.ttf`, `fonts/Jost-VariableFont_wght.ttf`, `fonts/Jost-Italic-VariableFont_wght.ttf`, `fonts/JetBrainsMono-VariableFont_wght.ttf` (+ italic) — wired via `@font-face` in `colors_and_type.css`. |
...
fonts/                   ← Manrope, Jost, JetBrains Mono variable TTFs
preview/                 ← design-system cards (color, type, spacing, components)
components/              ← exported DS components (Button, Input, Field, Badge, Card, Avatar) — importable by consuming projects
ui_kits/
...
templates/
  setting-form/          ← Core365 Template: Setting Form (master/detail settings form)
  form-with-tree-control/← Core365 Template: Form with Tree Control (hierarchical list rail)
```
...
1. **Fonts now wired locally.** Manrope, Jost, and JetBrains Mono variable TTFs are loaded from `fonts/` via `@font-face` in `colors_and_type.css`. (Google Fonts fallback removed.)
